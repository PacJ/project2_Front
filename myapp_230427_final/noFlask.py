...
for i in range(len(top_movies)):
    # print(i)
    top_movies.loc[i, 'OVERVIEW'] = remove_stopwords(preprocessing(okt_clean(top_movies['OVERVIEW'][i])))
...
tfidf_matrix = tfidf.fit_transform(top_movies['OVERVIEW'])
...
# #isbn의 인덱스 값을 가져오기
indices = pd.Series(top_movies.index, index = top_movies['OVERVIEW'])
...
def get_recommendations(movie_id, cosine_sim=cosine_sim) :
    ...
    movie_indices = pd.Series(top_movies.index, index=top_movies['MOVIE_ID'])
    # idx = indices[movie_id] 
    idx = movie_indices[movie_id]

    print(idx)
    print("indices[movie_id]")
    ...
    data = top_movies['MOVIE_ID'].iloc[movie_indices].to_list()
    response = requests.post('http://localhost:8090/recommend', json=data)
    print(response.status_code)
    print(response.json())
    # 인덱스를 이용해 MOVIE_ID추출 (list로 변환)
    # print(top_2000_movies.index[0])
    return top_movies['MOVIE_ID'].iloc[0:10].to_list()

get_recommendations(640146)
# get_recommendations(640146)
